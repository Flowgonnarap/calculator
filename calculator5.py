#    Made by Flow.
#    version = 5.1
from decimal import * #Using the decimal library to bring extremely accurate answers FOR FLOATS. {try doing 0.1 + 0.2 without the decimal library to understand what I mean.}
getcontext() 
        ...
        if A == 1:
            ...
            if avogadro_choice == 1:
                ...
                if choice == 1:
                    ...
                    if secondnum == 0: # This tries deleting secondnum if it is 0, to trigger the error at the try statement after it.
                        del secondnum
                    try:
                        secondnum
                    except NameError: # After the error of trying to locate the deleted secondnum vairable, it shows division by 0 is impossible.
                        print(f" \n \n{Fore.RED}Division{Style.RESET_ALL} by {Style.BRIGHT}0{Style.RESET_ALL} is {Style.BRIGHT}impossible.\n\n")
                    else:
                        total = (float(firstnum) / float(secondnum)) / Avogadronumber
